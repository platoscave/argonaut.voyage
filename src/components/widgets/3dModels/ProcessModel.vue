...
const drawPropositions = async (zPos) => {
  ...
  resArr.forEach((item) => {
    ...
    selectableMeshArr.push(propositionObj3d.children[0]);
...
const drawProcesses = async (zPos) => {
  ...
  resArr.forEach((item) => {
    ...
    selectableMeshArr.push(processObj3d.children[0]);
  });
  ...
  processArr.forEach((processObj3d) => {
    promisesArr.push(
      processObj3d.drawSteps(selectableMeshArr, glModelObj3d)
    );
  });
...
const drawOrgUnits = async (zPos) => {
  ...
  selectableMeshArr.push(rootOrgObj3d.children[0]);

  // Tell root class to draw the organizational units
  await rootOrgObj3d.drawOrgUnits(selectableMeshArr);
  ...
  uniqueActivePermAccArr.forEach((item) => {
    ...
    selectableMeshArr.push(userObj3d.children[0]);
